...
async def exchange_public_token(public_token_data: Dict[str, str], user: Any = Depends(get_current_user)):
    ...
    try:
        ...
        if response.data:
            ...
            return {"message": "Public token exchanged and data synced successfully!", "item_id": item_id}
        else:
...
async def sync_transactions(access_token: str, user_id: str, plaid_item_db_id: str):
    ...
    while has_more:
        try:
            ...
            for transaction in response.added:
                account_response = supabase.from_("accounts").select("id").eq("account_id", transaction.account_id).eq("plaid_item_id", plaid_item_db_id).single().execute()
                account_db_id = account_response.data["id"] if account_response.data else None
            ...
            for transaction in response.modified:
                ...
                if account_db_id:
                    ...
                    supabase.from_("transactions").update(transaction_data).eq("transaction_id", transaction.transaction_id).execute()
                    modified.append(transaction)

            # Remove deleted transactions
            for transaction in response.removed:
                supabase.from_("transactions").delete().eq("transaction_id", transaction.transaction_id).execute()
    ...
    print(f"Synced transactions: Added {len(added)}, Modified {len(modified)}, Removed {len(removed)}")
...
async def plaid_webhook(request: Request):
    try:
        ...
        print(f"Received Plaid webhook: Type={webhook_type}, Code={webhook_code}, Item ID={item_id}")

        # Retrieve access token and user_id from your database using item_id
        plaid_item_response = supabase.from_("plaid_items").select("access_token, user_id, id").eq("item_id", item_id).single().execute()
        if not plaid_item_response.data:
            raise HTTPException(status_code=404, detail="Plaid item not found in database")
        ...
        if webhook_type == "TRANSACTIONS":
            ...
            if webhook_code == "ERROR":
                ...
                supabase.from_("plaid_items").update({"status": "error", "error_code": error.get("error_code")}).eq("item_id", item_id).execute()
                return {"message": f"Item error: {error.get('error_code')}"}
            elif webhook_code == "PENDING_EXPIRATION":
                # Notify user to re-authenticate
                supabase.from_("plaid_items").update({"status": "pending_expiration"}).eq("item_id", item_id).execute()
